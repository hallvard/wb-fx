...
public class ViewManager implements Configurable {
    ...
    private Optional<MenuItem> createInfoMenuItem(ViewProvider viewProvider, ViewInfo viewInfo) {
        ...
        // TODO: remove implicit dependency on MarkdownViewController
        ...
        menuItem.setOnAction(event -> createView("no.hal.wb.views.markdown.MarkdownView", configuration));
        return Optional.of(menuItem);
    }
    ...
    public Map<String, List<MenuItem>> createViewCreationMenuItems(String textFormat) {
        var viewByCategory = viewProviders.stream()
            .filter(viewProvider -> viewProvider.getViewInfo().viewCategory() != null)
            .collect(Collectors.groupingBy(viewProvider -> viewProvider.getViewInfo().viewCategory()));
